...
class Graph: #from colab
    ...
    def breadth_first_search(self, s): #from colab
    ...
    def depth_first_search(self): #from colab
        for v in self.vertList.values():
            v.color = 'white'
    ...
    def DFS(self, vid, path): #from colab
        v = self.vertList[vid]
        v.color = 'gray'
        path.append(v.id)
        for u in v.connectedTo.keys():
